...
# ~~~~~~~~~~~~~~~~~ load data and properties of matching source ~~~~~~~~~~~~~~~~~~
    ...
    if atlas_w20 < V_turb:
        V_turb = atlas_w20
    incl_v_scale = np.sqrt( V_turb**2+(np.sqrt(atlas_w20**2-V_turb**2)*(np.sin(ska_incl)/np.sin(incl)))**2)/atlas_w20
   ...
   # flux_integral = ska_M_HI/(2.36e5*ska_D_L**2) # equal to int S dV, units jy-km/s
    ...
    return cube6, ska_D_HI_kpc_direct, ska_D_HI_arcsec_direct, ska_D_HI_kpc, ska_D_HI_arcsec, i[8], is_unresolved, flux_integral, ska_w20, MHI_incl_scale,MHI_incl_v_scale, ska_PA  # sources are now centred on dynamical centre of galaxy (according to crpix values in original_blanked_cubes) so dont need to pass crpix values (assuming centred)
