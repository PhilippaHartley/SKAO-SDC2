...
def fsigmoid(x, a, b, k):
    return (a+(  (k-a) /  (1.0 + np.exp(-b*x )  )   ) ) # could generalise with 1/nu as an index to denominator
...
def make_cube(config,i,ska_M_HI,ska_Mh,ska_z,ska_incl,ska_PA,ska_dx, dnu, psf_maj, radioclass, optclass):
   ...
   # print ('optclass: ', optclass)
    ...
    normalised_atlas_M_HI = (np.log10(atlas_M_HI) - np.min(np.log10(atlas_M_HI) ) )/(np.max(np.log10(atlas_M_HI) )- np.min(np.log10(atlas_M_HI) ))
    normalised_atlas_incl = (np.cos(atlas_incl) - np.min(np.cos(atlas_incl)) )/(np.max(np.cos(atlas_incl))- np.min(np.cos(atlas_incl)))

    normalised_ska_M_HI = (np.log10(ska_M_HI) - np.min(np.log10(atlas_M_HI)) )/(np.max(np.log10(atlas_M_HI))- np.min(np.log10(atlas_M_HI)))
    normalised_ska_incl = (np.cos(ska_incl) - np.min(np.cos(atlas_incl)) )/(np.max(np.cos(atlas_incl))- np.min(np.cos(atlas_incl)))
   
    #should no HI sources go into AGN-- agn sources still have hi_pred



    distance_to_sample = np.hypot(normalised_ska_incl - normalised_atlas_incl,normalised_ska_M_HI - normalised_atlas_M_HI)
...
# ~~~~~~~~~~~~~~~~~ load data and properties of matching source ~~~~~~~~~~~~~~~~~~
    ...
    distance = float(i[2]) # Mpc # this is irrelevant for data since flux scale incorrect and pixels resized, but may be relevabt for derived proprties
    sys_vel = float(i[3])*1000 # m/s, should be similar to rec vel but will include pec vel
    incl = (float(i[4])/360.)*2*np.pi
    rot_vel = float(i[5]) # this is corrected for inclination, according to hyperleda from which it was taken
    pa = float(i[6])*deg2rad # make a new column for this : pas are in hyperleda thing
    ...
    ska_dz = tools.get_spectral_sampling(config, rest_freq, central_freq, dnu) # optical velocity interval
    ...
    ska_D_HI_kpc = 10**log10_ska_D_HI #kpc; retain for comaprison with D_HI measured from atlas cube
    ska_D_HI_arcsec = (ska_D_HI_kpc/(ska_D_A*1000))*rad2arcsec # convert D_A to kpc
    ...
    logging.info('Atlas D_HI directly measured, kpc: %f', atlas_LAS_maj) # ususally in agreement, sometimes up to a factor of ~1.7 out
    ...
    MHI_scale = 10**(log10_ska_D_HI)/10**(log10_atlas_D_HI) # is equal to M_HI_factor**0.51
    ...
    # use rad_vel(r,theta) = rot_vel(r)*cos(theta)*sin(i)
    # rotation velocity is measured at r = r and theta = 0;
    # rot_vel = rad_vel(r)/sin(i)
    incl_v_scale = np.sin(ska_incl)/np.sin(incl)
    ...
    atlas_physical_pixel_size = 1000*1e-6 # Mpc # this should come from fits header - put in

   # D_A = physical_size/angular_size
   # angular size is in radians
   # physical size and D_A in Mpc

    angular_pixel_scale_at_given_redshift = (atlas_physical_pixel_size/ska_D_A)*rad2arcsec

    logging.info('Angular scale (of a 100Mpc pixel) at given redshift, arcsec: %f', angular_pixel_scale_at_given_redshift)
    ...
    redshift_v_scale = dz/ska_dz # v resizing is due to change in v resolution - which does vary with z



    logging.info('Scalings due to moving to redshift:')
    logging.info('Scale factor along both spatial axes: %f', redshift_scale)
    logging.info('Velocity scale factor (due to change of v resolution only): %f', redshift_v_scale)
    ...
    logging.info('Combined scale factors due to mass and inclination: %f, %f, %f', MHI_v_scale*incl_v_scale, MHI_scale*incl_scale, MHI_scale)
    logging.info('Combined scale factors due to mass and inclination and z: %f, %f, %f', total_v_factor, total_b_factor, total_a_factor)



    print (new_v_size,new_a_size,new_b_size)
    ...
    while np.ceil(new_a_size) %2 == 0:        
        ...
        npad=int((1./total_a_factor/2.)) #pixels in original size to correspond to 0.5 pixel in the new image
       
        cube = np.pad(cube, ((0,0),(npad,npad), (0,0)), 'constant', constant_values=(0))
    ...
    while np.ceil(new_b_size) %2 == 0:
        ...
        print ('is even in b')
        # a bit of padding on both sides to leave the image centered as before but to get an odd number of pixels in the end
        npad=int(1./total_b_factor/2.) #pixels in original size to correspond to 0.5 pixel in the new image
        print('padding on both sizes of',npad)
      
        cube = np.pad(cube, ((0,0),(0,0), (npad,npad)), 'constant', constant_values=(0))
    ...
    cube2 = transform.resize(cube, (np.ceil(new_v_size),np.ceil(new_a_size),np.ceil(new_b_size)), order = interp_order ,preserve_range = True, anti_aliasing=True)
    cube_summed_spectrum = transform.resize (cube_summed_spectrum, (np.ceil(new_v_size),1), order = interp_order ,preserve_range = True, anti_aliasing=True)

    logging.info('Subcube shape after scaling for mass and inclination and z: %s', cube2.shape)
    ...
    ska_LAS_maj_pix = (ska_LAS_maj_kpc/(atlas_physical_pixel_size*1000))*redshift_scale#1000 converts to kpc
    ska_LAS_min_pix = (ska_LAS_min_kpc/(atlas_physical_pixel_size*1000))*redshift_scale
    ...
    padding = 5
    cube4 = np.pad(cube2, ((0,0),(padding,padding), (padding,padding)), 'constant', constant_values=(0)) # need to sort this padding out     
    ...
    flux_integral = ska_M_HI / (49.8 * ska_D_L**2) # (Duffy+12, converted from below via c amd rest freq.. sampling is now fixed freq, should give same result? yes)
   # flux_integral = ska_M_HI/(2.36e5*ska_D_L**2) # equal to int S dV, units jy-km/s
    ...
    ska_summed_flux = flux_integral/(dnu) # this enables normalisation to correct Jy value per velocity slice 
    ...
    cube6_ska_summed_flux = np.sum(cube6)
    logging.info('Subcube flux after normalisation for pixels of beam: %f', cube6_ska_summed_flux) 
    ...
    return cube6, ska_D_HI_kpc_direct, ska_D_HI_arcsec_direct, ska_D_HI_kpc, ska_D_HI_arcsec, i[8], is_unresolved, flux_integral, ska_w20, MHI_incl_scale,MHI_incl_v_scale  # sources are now centred on dynamical centre of galaxy (according to crpix values in original_blanked_cubes) so dont need to pass crpix values (assuming centred)
...
if __name__=='__main__':
    ...
    if make == 1:
        ...
        pixel_scale = ska_bmin/2.
        velo_resolution = 5000

        pixel_scale = ska_bmin/3. # get these from input file
        ...
        ska_incl = (40/ 360.)*2 *np.pi

        ska_PA = 70 # degrees anti-clockwise from N-S (y axis)

        ska_radioclass = 1

        sub_cube, ska_D_HI, ska_D_HI_arcsec,atlas_source=make_cube(config,ska_M_HI,ska_Mh,ska_z,ska_incl,ska_radioclass,ska_PA, pixel_scale, velo_resolution)
