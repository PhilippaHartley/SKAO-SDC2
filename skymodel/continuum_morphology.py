...
def make_img(config,ska_flux,ska_alpha,ska_freqmin,freqs,ska_size,ska_min,ska_PA,ska_dx, psf_maj, radioclass,corefrac,ska_ranid):
...
#    ska_incl = ska_incl*deg2rad
    ...
    if (radioclass <4):
        Gaussize_in_pixels=Gaussize_in_pixels*1.4241 #scale lenght to FHWM - true for SFGs
        Gauss_minor_size_in_pixels=Gaussize_in_pixels*ska_min/ska_size #minor axis
        
    if (radioclass == 6):
        Gaussize_in_pixels=size_in_pixels/3. #LAS approximated with a 3sigma - it varies for different images and true distribution is not Gaussian 
        Gauss_minor_size_in_pixels=Gaussize_in_pixels # initialised for a circularly symmetric source


    ska_ellip=1.-ska_min/ska_size
    beamsigma_in_pixels=psf_maj/sigma2FWHM/ska_dx


    #initialization for unresolved source

    sigma, mu = beamsigma_in_pixels, 0.0  #exponential scale lenght converted to sigma in pixels

    npixs=int(sigma*8.)+1
    x, y = np.meshgrid(np.linspace(-npixs/2.,npixs/2.,npixs), np.linspace(-npixs/2.,npixs/2.,npixs))
    ...
    if (Gauss_minor_size_in_pixels>3.) and (radioclass<4):
    ...
    if (Gaussize_in_pixels>3.) and (radioclass>=4) and (radioclass<6):
    ...
    if (radioclass==6) and (Gaussize_in_pixels>3.):
...
#            padding_b=cube.shape[1]
        ...
        if (np.ceil(halfs) == np.floor(halfs)):
            ...
            npad=int(1./zoom_factor/2.) #pixels in original size to correspond to 0.5 pixel in the new image

            print('padding on both sizes of',npad)
            cube = np.pad(cube, ((npad,npad), (npad,npad)), 'constant', constant_values=(0))
        ...
        cube2 = transform.resize(cube, (new_a_size, new_b_size), order = interp_order ,preserve_range = True)
        ...
        psf_smooth=np.sqrt(np.square(psf_maj/ska_dx)-np.square(atlas_psf*zoom_factor/ska_dx)) #additional smoothing PSF in units of the map pixel
        ...
        if (psf_smooth > 0. ):
            ...
            cube2_padded = np.pad(cube2, ((padding_a,padding_a), (padding_b,padding_b)), 'constant', constant_values=(0))

            astropy_conv = convolve(cube2_padded, atlas_kernel,boundary='extend', normalize_kernel=True)
            
        
            cube2 = astropy_conv[padding_a:padding_a+size_a,padding_b:padding_b+size_b]
    ...
    if (smoothing_done ==0):
        ...
        astropy_conv = convolve(cube2_padded, kernel,boundary='extend', normalize_kernel=True)
    ...
    for ff in range(nfreqs):
        norm=ska_flux*(freqs[ff]/ska_freqmin)**ska_alpha  #flux mormalization for frequency freq
    ...
    cube6= scipy.ndimage.rotate(cube5, ska_PA-90,  axes = (1,2),reshape=False)# subract 90 to give PA anti-clockwise from North
  ...
  #  cube6 = np.ones((5,100,100))
    return cube6, cube_name, is_unresolved, ska_flux  # sources are now centred on dynamical centre of galaxy (according to crpix values in original_blanked_cubes) so dont need to pass crpix values (assuming centred)
