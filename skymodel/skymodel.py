...
inclinatio
some radioclass AGN are opticalclass SFG?
I### refers only to continuum?
'''
import numpy as np
...
def add_source(i, cat_gal, nobj, w_spectral, config, pixel_scale_str, dnu, psf_maj_arcsec, arr_dims, all_gals_fname, cat):
  
    logging.info('..........Adding source {0} of {1} to skymodel..........'.format(i+1, nobj))

    x, y, v = w_spectral.wcs_world2pix(cat_gal['RA'], cat_gal['Dec'], cat_gal['opt_vel'], 1) 
#    for three axes:   pixels = wcs.world_to_pixel(coord, 3000 * u.m / u.s)  

    logging.info('RA, Dec, opt_vel: %f %f %f  ',cat_gal['RA'], cat_gal['Dec'], cat_gal['opt_vel'])
    ...
    sub_cube, D_HI,  D_HI_arcsec, D_HI_relation,  D_HI_arcsec_relation,atlas_source, unresolved, flux, w20, MHI_incl_scale,MHI_incl_v_scale, ska_PA \
                     = make_cube(config, i,10**cat_gal['MHI'],10**cat_gal['Mh'],cat_gal['z'],cat_gal['i'],\
                     cat_gal['PA'], float(pixel_scale_str), dnu, psf_maj_arcsec, cat_gal['RadioClass'], cat_gal['OptClass'])  
   # unresolveds+=unresolved
    ...
    l_bounds = np.array([v,y,x])-np.array([(sub_cube.shape[0]/2), (sub_cube.shape[1]/2), (sub_cube.shape[2]/2)])
    u_bounds = np.array([v,y,x])+np.array([sub_cube_shape[0]-(sub_cube.shape[0]/2), sub_cube_shape[1]-(sub_cube.shape[1]/2),sub_cube_shape[2]-(sub_cube.shape[2]/2)])
    ...
    top_excess = arr_dims-np.array([trc0, trc1, trc2])
    excess = np.hstack((np.array([blc0, blc1, blc2]),top_excess))
    overlap=False
    for coord in excess:
        if (coord<1):
...
def runSkyModel(config):
    ...
    data_path_large_files =  config.get('pipeline', 'data_path_large_files')+config.get('pipeline', 'project_name')+'/'
    ...
    logging.info('Final cropped min and max velocities, m/s: %f %f'%(velo_base, velo_max))
    arr_dims = np.array([n_chan, image_size, image_size])

    logging.info('Final array dimensions: %d %d %d'%(n_chan, image_size, image_size))
    logging.info('Final array size, elements: %.3e'%(n_chan*image_size*image_size))
    logging.info('Final array size, bytes: %.3e'%(n_chan*image_size*image_size*4))
    ...
    pad_image_size, pad_fov, pad_base_freq, pad_top_freq, cut_base_freq, cut_top_freq = tools.add_padding(image_size, fov, base_freq, top_freq, pixel_scale, dnu)
    ...
    logging.info('Source cut min and max velocities, m/s: %f %f'%(cut_velo_base, cut_velo_max))
  ...
  #  header_fourd = w_fourd.to_header()
    ...
    all_gals_fname = data_path_large_files+config.get('field', 'fits_prefix')+'.fits'
    ...
    logging.info('Padded array dimensions: %d %d %d'%(pad_n_chan, pad_image_size, pad_image_size))
    logging.info('Padded array size, elements: %.2e'%(pad_n_chan*pad_image_size*pad_image_size))
    logging.info('Padded array size, bytes: %.2e'%(pad_n_chan*pad_image_size*pad_image_size*4))
    ...
    fitsf[0].write(img2, blc2, blc1, blc0 ,blc2+img2.shape[2]-1,  blc1+img2.shape[1]-1, blc0+img2.shape[0]-1)

    fitsf.close()

    cr_x, cr_y = w_twod.wcs_world2pix(ra_field_gs, dec_field_gs, 1,) 
    ...
    if config.getboolean('skymodel', 'doHI'):
      ...
      #  f= open(outf,"w+")
        ...
        source_prefix_arr = strmultiply(source_prefix, np.ones_like(source_name_ra, dtype=int))
        ...
        source_name_pos = stradd(source_prefix_arr, stradd(source_l_arr, stradd(source_name_ra, (stradd(source_b_arr, source_name_dec)))))
        ...
        cat['line_flux_integral'].unit = 'Jy Hz' # using freq version of MHI-line flux conversion (Duffy+12)
        ...
        logging.info('ra offset min max: %f %f', np.min(cat['ra_offset']), np.max(cat['ra_offset']))
        logging.info('dec offset min max: %f %f', np.min(cat['dec_offset']), np.max(cat['dec_offset']))
        ...
        ra_offset_max = (1/cosdec)*((pad_fov/60)/2)  # this is now a fraction of the written image size
    ...
    #    unresolveds = 0
        ...
        if doplot:
            H, xedges, yedges = np.histogram2d( (cat['MHI']),np.cos(cat['i']/180*np.pi)*10)#, bins=(xedges, yedges))   
        ...
        for i,cat_gal in enumerate(cat): 
            add_source(i,cat_gal, nobj, w_spectral, config, pixel_scale_str, dnu, psf_maj_arcsec, arr_dims, all_gals_fname, cat )
           # if i == 100000:
           #     break
        '''
        ...
        truthcat_name = data_path+config.get('field', 'fits_prefix')+'_restricted_truthcat.fits'
        logging.info('Writing restricted truth catalogue to: {0} ...'.format(truthcat_name))
    ...
    #    public_cat['HI_size_kpc'].unit = 'kpc'
        ...
        public_cat['line_flux_integral'].unit = 'Jy Hz' # using freq version of MHI-line flux conversion (Duffy+12)
    ...
    #    public_cat['z'].unit = 'none'
        ...
        public_truthcat_name = data_path+config.get('field', 'fits_prefix')+'_public_truthcat.fits'
        logging.info('Writing public truth catalogue to: {0} ...'.format(public_truthcat_name))
    ...
    if config.getboolean('primarybeam', 'dopb'):
        ...
        if config.getboolean('skymodel', 'dosimple_psf'):
            ...
            noise_sigma = image_data.max()/config.getfloat('skymodel', 'simple_psf_snr')
            noise_data = np.random.normal(loc=0., scale=noise_sigma, size=image_data.shape)
            image_data = image_data + noise_data
            
            hdu_noise = astfits.PrimaryHDU(np.expand_dims(np.expand_dims(noise_data, axis=0), axis=0), header=header_spectral)
            ...
            psf_image = galsim.ImageF(image_size, image_size, scale=pixel_scale/galsim.arcsec)
            psf.draw(psf_image)
            psf_data = psf_image.array
            hdu_psf = astfits.PrimaryHDU(np.expand_dims(np.expand_dims(psf_data, axis=0), axis=0), header=header_spectral)
        ...
        hdu = astfits.PrimaryHDU(np.expand_dims(np.expand_dims(image_data, axis=0), axis=0), header=header_spectral)
    ...
    if config.getboolean('skymodel', 'im3cat'):
        np.savetxt(data_path+config.get('pipeline', 'project_name')+'_im3cat.txt', np.column_stack([np.arange(nobj), ix_arr, iy_arr]))
