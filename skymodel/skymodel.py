...
def runSkyModel(config):
    ...
    if config.getboolean("skymodel", "doHI"):
        ...
        print,cat_read.colnames(cat_read)
    ...
    if config.getboolean("skymodel", "doagn"):
        ...
        if config.getboolean("skymodel", "doagn"):
            ...
            pa = cat_read["PA"]  # this is the HI PA. rotate of 90 degs for AGN counterparts

            if (HI_cross == True):
                cat["MHI"] = cat_read["MHI"]  # this needed to select only Hi counterparts - for test purposes
